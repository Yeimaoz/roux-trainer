...
export function FbLesson({ onNext }: { onNext?: () => void }) {
  return (
    <article className="lesson">
      ...
      <p>
        橋式解法的標準持法是：<strong>黃色底面朝下、橙色面朝左、綠色面朝前</strong>
        （標準 WCA 配色：白對黃、橙對紅、綠對藍；白色在頂、黃色在底）。
        第一橋就建在左下側（橙色左面 + 黃色底面的交界處）。
      </p>
      ...
      <p>
        DL 邊的辨識非常直觀——它是黃橙雙色邊（黃色底面、橙色左面的交界邊）。
        找到它後，根據它目前的位置：
      </p>
